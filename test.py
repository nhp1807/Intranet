...
def main(config):
    ...
    for i_exp in range(config.n_exp):
        model = models.RQ_VQA(None)
        model = torch.nn.DataParallel(model, device_ids=config.gpu_ids)

        print('load the model ' + os.path.join(config.pretrained_path, model_file[i_exp]))
        model.load_state_dict(torch.load(os.path.join(config.pretrained_path, model_file[i_exp]), map_location=torch.device('cpu')))
